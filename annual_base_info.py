...
class spiders(object):
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            print('')
            print(result_dict['baseInfo'])
            print('')
            print(result_dict)
            print(self.deal_json('items',result_dict))
            for result in json.loads(result_dict):
                print('sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssaaaaaaaa')
                print(result['items'])
                print('')
                print(result['baseInfo'])
                print('')
                print(result)
                print(self.deal_json('items', result))
                print('sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssaaaaaaaa')
                result_frame.append([
                    ...
                    self.deal_json('reportYear', result),
                    self.deal_json('companyName', result),
                    self.deal_json('creditCode', result),
                    self.deal_json('regNumber', result),
                    self.deal_json('phoneNumber', result),
                    self.deal_json('postcode', result),
                    self.deal_json('postalAddress', result),
                    self.deal_json('email', result),
                    self.deal_json('manageState', result),
                    self.deal_json('employeeNum', result),
                    self.deal_json('operatorName', result),
                    self.deal_json('totalAssets', result),
                    self.deal_json('totalEquity', result),
                    self.deal_json('totalSales', result),
                    self.deal_json('totalProfit', result),
                    self.deal_json('primeBusProfit', result),
                    self.deal_json('retainedProfit', result),
                    self.deal_json('totalTax', result),
                    self.deal_json('totalLiability', result)
                ])
            return result_frame
